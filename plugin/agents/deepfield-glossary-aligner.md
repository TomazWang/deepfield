...
- **Drafts directory** — path to `deepfield/drafts/` (contains `behavior/` and `tech/` subtrees)
...
List all `*.md` files under `deepfield/drafts/behavior/` and `deepfield/drafts/tech/` recursively (including all spec files inside domain subdirectories). Exclude `README.md` files. For each draft file:
...
deepfield upgrade:apply-op --type update \
  --path "drafts/<track>/<domain>/<file>.md" \
...
|------|---------------|-----------------|-------|
| drafts/behavior/auth/spec.md | JWT | auth token | 3 |
| drafts/tech/api/spec.md | JWT | bearer token | 1 |

---

## Skipped (Potential Manual Review)

- Skipped plural form "auth tokens" in drafts/behavior/auth/spec.md (potential manual review item)

---

## No Changes Required

- drafts/tech/database/spec.md — no synonyms found
```
...
- NEVER modify `README.md`, `_changelog.md`, `unknowns.md`, or any file outside `deepfield/drafts/behavior/` and `deepfield/drafts/tech/`
