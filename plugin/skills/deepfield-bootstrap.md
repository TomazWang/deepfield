...
5. Generates `deepfield/wip/tech-index.md` (tech domain detection from source code)
...
## Step 3b: Dual-Track Domain Detection

After the bootstrap runner script completes and `wip/tech-index.md` exists, perform behavior domain detection and write `wip/behavior-index.md`.

### Step A: Tech domain detection

The bootstrap runner script (Step 3) already writes `deepfield/wip/tech-index.md` based on source code structure. No additional action required here — tech domains are complete.

### Step B: Behavior domain detection
...
If the `--skip-behavior-qa` flag was **not** passed:
...
Wait for the user's response. Incorporate any additions or removals into the confirmed domain list.

**If no candidates were found:**

```
I could not detect behavior domains from the available documentation.

What product features do your stakeholders care about?
List the main capabilities (one per line).
```

Wait for the user's response. Use the provided list as the confirmed domain list.

If the `--skip-behavior-qa` flag **was** passed:
- Use the candidates list as-is (empty list if no candidates were found)
- Skip the interactive prompt entirely

### Step D: Write behavior-index.md

Using the confirmed domain list from Step C (or the candidates list if `--skip-behavior-qa`), write `deepfield/wip/behavior-index.md` atomically.

Check whether `plugin/templates/behavior-index.md` exists:

```bash
ls "${CLAUDE_PLUGIN_ROOT}/templates/behavior-index.md" 2>/dev/null
```

If the template exists, use it as the structure for the file, substituting the confirmed domain list. If it does not exist, write the file in this format:

```markdown
# Behavior Domain Index

Generated by: Run 0 bootstrap
Date: <ISO date>

## Detected Behavior Domains

| Domain | Description | Source |
|--------|-------------|--------|
<one row per confirmed domain>

## Notes

- Domains represent product features and capabilities from a stakeholder perspective
- Update this index after each run as new behavior domains are discovered
```

Write atomically (temp file → rename) to `deepfield/wip/behavior-index.md`.
...
When generating the tech domain index and initial domain notes, for each detected tech domain check whether `deepfieldConfig.domainInstructions[domainName]` exists. If it does, include those instructions in the context when the AI analyzes that domain.
...
Also verify that `deepfield/wip/domain-index.md` does **not** exist — bootstrap must not create the old single-track domain index.
...
Review the generated files:
  ...
  - deepfield/wip/tech-index.md       <- Tech domains (from source code)
  - deepfield/wip/behavior-index.md   <- Behavior domains (stakeholder view)
  - deepfield/wip/domain-links.md     <- Behavior ↔ Tech domain links
...
- `deepfield/wip/tech-index.md` exists
- `deepfield/wip/behavior-index.md` exists
