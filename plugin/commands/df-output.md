...
After the snapshot is written (Steps 1–7), generate a ready-to-run Docusaurus site at:

```
deepfield/output/[version]/docusaurus/
```

#### 8a. Read project config

Read `deepfield/project.config.json` to extract:
- `projectName` → use as `{{PROJECT_NAME}}`
- Derive `{{PROJECT_SLUG}}` by lowercasing and replacing spaces/non-alphanumeric with `-`

#### 8b. Copy static template files

Copy these files from `plugin/templates/docusaurus/` (substituting `{{PROJECT_NAME}}`, `{{PROJECT_SLUG}}`, `{{SNAPSHOT_VERSION}}`, `{{SNAPSHOT_DATE}}`):

```
docusaurus/
├── package.json          ← substitute PROJECT_SLUG
├── docusaurus.config.js  ← substitute PROJECT_NAME, SNAPSHOT_VERSION, SNAPSHOT_DATE
├── src/css/custom.css    ← copy as-is
└── README.md             ← substitute PROJECT_NAME, SNAPSHOT_VERSION, SNAPSHOT_DATE
```

#### 8c. Mirror snapshot markdown into docs/

For every `.md` file in `${OUTPUT_DIR}/` (the snapshot), copy it into `docusaurus/docs/` preserving subdirectory structure, **then prepend Docusaurus frontmatter** if none already exists:

**Frontmatter rules:**
- `title`: derive from the first `# Heading` in the file; fall back to the filename (title-cased, dashes→spaces)
- `sidebar_label`: same as `title` but shorter — strip the domain name prefix if it repeats (e.g. `"Auth Service — Technical Design"` → `"Technical Design"`)
- `sidebar_position`: assign 1, 2, 3… in alphabetical order within each directory
- Skip `SNAPSHOT.md` — it becomes `docs/intro.md` (the landing page) with `sidebar_position: 1`
...
#### 8d. Generate sidebars.js

Scan the `docusaurus/docs/` directory tree and generate `sidebars.js`:
...
const sidebars = {
  knowledgeSidebar: [
    { type: 'doc', id: 'intro', label: 'Snapshot Overview' },
    {
      type: 'category',
      label: 'Context',           // maps to docs/context/ tier
      collapsed: false,
      items: [{ type: 'autogenerated', dirName: 'context' }],
    },
    {
      type: 'category',
      label: 'Technical Design',  // maps to docs/tech-design/ tier
      collapsed: false,
      items: [{ type: 'autogenerated', dirName: 'tech-design' }],
    },
    {
      type: 'category',
      label: 'Implementation',    // maps to docs/implementation/ tier
      collapsed: true,
      items: [{ type: 'autogenerated', dirName: 'implementation' }],
    },
    {
      type: 'category',
      label: 'Cross-Cutting',     // maps to docs/cross-cutting/
      collapsed: true,
...
**Adaptation rules:**
- Only include a category if that directory exists under `docs/`
- For the old flat `domains/` structure (pre-3-tier), use a single `"Domains"` category instead:
  ```js
  { type: 'category', label: 'Domains', items: [{ type: 'autogenerated', dirName: 'domains' }] }
  ```
- If none of the known tiers are detected, fall back to full autogeneration:
  ```js
  knowledgeSidebar: [{ type: 'autogenerated', dirName: '.' }]
  ```

#### 8e. Create docs/intro.md

Rename/copy `SNAPSHOT.md` into `docusaurus/docs/intro.md` and ensure it has this frontmatter:
...
#### 8f. Write .gitignore for Docusaurus

Create `docusaurus/.gitignore`:
...
- The Docusaurus site is self-contained — users can `cd` into it and run `npm start` immediately
- Frontmatter is only added if missing — existing frontmatter in draft files is preserved
- Sidebars auto-adapt to whichever tier structure the workspace uses (3-tier or legacy flat)
