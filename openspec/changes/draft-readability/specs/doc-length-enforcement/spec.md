...
### Requirement: AI agents observe 200-line maximum for draft documents
AI agents generating or updating draft documents in `deepfield/drafts/` SHALL keep each document to a maximum of approximately 200 lines. When a document would exceed 200 lines, the agent SHALL split it following the splitting guidelines.

#### Scenario: Agent respects limit when creating new draft
- **WHEN** an agent creates a new domain draft file
- **THEN** the file contains no more than ~200 lines of content

#### Scenario: Agent splits document when update would exceed limit
- **WHEN** an agent attempts to add content that would push a domain draft past 200 lines
